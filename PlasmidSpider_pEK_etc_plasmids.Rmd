...
chrom <- input$Species
length(chrom) # 4445
# Escherichia_coli_536_	818
# Escherichia_coli_ATCC_8739_	1153
# Escherichia_coli_BL21	1108
# Escherichia_coli_CFT073_	355
# Escherichia_coli_O157:H7_str._EDL933_	279
# Escherichia_coli_str._K-12_substr._MG1655_	1211
# Cellvibrio_sp._BR_	3041
# Pseudomonas_fluorescens_F113_	2566
# Pseudomonas_aeruginosa_	1
# Aromatoleum_aromaticum_EbN1_	187
# Klebsiella_oxytoca_	10
# Klebsiella_pneumoniae_	11
# Shigella_flexneri_2a_str._301_	351
# Methylibium_petroleiphilum_PM1_	1003
# Rubrivivax_benzoatilyticus_JA2_=_ATCC_BAA-35_	2244
# Ralstonia_eutropha_H16_	852
...
for (k in o){           # change k  1:4445
    ...
    #  write.csv(links_all, paste("CHROM_DATA/", input$Species[k], "_chrom_data.csv", sep=""))
   ...
   ######## get plasmid's gene interactions # select plasmid-associated interactions
    for (jj in 1:length(accessID2)){
      ...
      #write.csv(mapped_p, paste("CSV_FIMPAPIL/", input$Species[k], "_", access_ID2, ".csv", sep=""))
 ...
 #pdf(paste("PLASMID_CHROM_FIMPAPIL/", input$Species[k], "_",
 #access_ID2,"_plasmid-chrom_subnetwork.pdf", sep=""),     width=16, height=20)
#      string_db$plot_network(string_db$mp(unique(c(mapped_p$from_ID, mapped_p$to_ID)))) # make plot 
 #     dev.off()    # make a plot of the network for fun
      if(jj==1){ 
  pdf(paste("INTRAPLASMID_FIMPAP/", input$Species[k], "_",
      access_ID2, "_intraplasmid_subnetwork.pdf", sep=""),width=16, height=20)
      string_db$plot_network(string_db$mp(p1)) # make plot 
      dev.off()  }  # make a plot of the network for fun
   if(jj==2){    
  pdf(paste("INTRAPLASMID_PIL/", input$Species[k], "_", access_ID2,
            "_intraplasmid_subnetwork.pdf", sep=""),width=16, height=20)
      string_db$plot_network(string_db$mp(p1)) # make plot 
      dev.off()  }  # make a plot of the network for fun
...
tail(table_plasmid2)
input$Species[k]
write.csv( table_plasmid2, "Table_Snew_plasmids_species_FIMPAPIL.csv")
k
...
table_jaccard <- cbind(table_jaccard, 
   setNames(data.frame(matrix(ncol = 4, nrow = 0)), table_plasmid$Plasmid[1:4]))
...
View(table_jaccard)
write.csv(table_jaccard,"table_jaccard_species.2.csv") # write out to CSV
...
for (i in 1:length(chrom)) { #add in interactions
  table_interaction[length(table_interaction$Chrom)+1,] <-
    c(chrom[i], as.numeric(getchrom.plasmid(chrom[i])$Interactions) ) } # end if 
#table_interaction <- table_interaction[, colSums(table_interaction != 0) > 0]
View(table_interaction)
write.csv(table_interaction,"table_interaction_species.pEK.csv") # write out to CSV
...
for (i in 1:length(chrom)) { #add in interacting genes
  table_interacting_genes[length(table_interacting_genes$Chrom)+1,] <-
    c(chrom[i],  as.numeric(getchrom.plasmid(chrom[i])$Interacting_genes)) }
table_interacting_genes <- table_interacting_genes[, colSums(table_interacting_genes != 0) > 0]
View(table_interacting_genes)
write.csv(table_interacting_genes,"table_interacting_genes_species.pEK.csv")
